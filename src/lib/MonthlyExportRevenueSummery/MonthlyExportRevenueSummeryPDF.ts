...
export interface MonthlyRevenueLeakageData {
  id: number | string;
...
export function MonthlyExportRevenueSummeryPDF(
  items: MonthlyRevenueLeakageData[],
  totalLeakage: number,
  growth: number,
  monthLabel: string,
  filters: {
  ...
  doc.text(
    `Monthly Revenue Leakage Analysis • ${monthLabel}`,
    margin,
    24
  );
  ...
  for (
    ...
    doc.text(
      "Hospital Management System • Revenue Leakage Monthly MIS",
      margin,
      pageHeight - 6
    );
  ...
  doc.save(
    `Monthly_Revenue_Leakage_Summary_${
      safeMonthLabel || "MIS"
    }.pdf`
  );
}
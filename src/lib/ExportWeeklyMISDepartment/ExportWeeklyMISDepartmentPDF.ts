...
export function ExportWeeklyMISDepartmentPDF(
  ...
  doc.text(
    `Weekly Department Performance Overview • ${weekLabel}`,
    margin,
    24
  );
  ...
  for (
    ...
    doc.text(
      "Leads Health Care • Department-wise Weekly MIS",
      margin,
      pageHeight - 6
    );
  ...
  doc.save(
    `Weekly_Department_MIS_${safeWeekLabel}.pdf`
  );
}
...
export function ExportWeeklyDoctorPerformancePDF(
  doctors: WeeklyDoctorData[],
  weekLabel: string,
  filters: {
    department: string;
    rating: string;
  ...
  doc.text(
    `Weekly Doctor Performance Overview • ${weekLabel}`,
    margin,
    24
  );
  ...
  for (
    ...
    doc.text(
      "Leads Health Care • Doctor Performance Weekly MIS",
      margin,
      pageHeight - 6
    );
  ...
  doc.save(
    `Weekly_Doctor_Performance_${safeWeekLabel}.pdf`
  );
}